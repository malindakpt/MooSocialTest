...
class PhotoHelper extends AppHelper {	
	...
	public function getImage($item, $options) {
            ...
            if ($item['Photo']['thumbnail']) {
                if ($item['Photo']['year_folder']){  // hacking for MOOSOCIAL-2771 
                    $year = date('Y', strtotime($item['Photo']['created']));
                    $month = date('m', strtotime($item['Photo']['created']));
                    $day = date('d', strtotime($item['Photo']['created']));
                    $url = FULL_BASE_URL . $request->webroot . "uploads/photos/thumbnail/$year/$month/$day/" . $item['Photo']['id'] . '/' . $prefix . $item['Photo']['thumbnail'] . '?' . time();
                }else{
                    $url = FULL_BASE_URL . $request->webroot . 'uploads/photos/thumbnail/' . $item['Photo']['id'] . '/' . $prefix . $item['Photo']['thumbnail'] . '?' . time();
                }
