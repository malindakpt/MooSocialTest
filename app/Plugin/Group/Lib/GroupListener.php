...
class GroupListener implements CakeEventListener {
    ...
    function afterSetParamsConditionsOr($event)
    {
        ...
        $public_groups = Cache::read($key,"group");
        if (!$public_groups)
        {       
        	$conditions =  array(
           		'Group.type' => array(PRIVACY_PUBLIC,PRIVACY_RESTRICTED),
            );
            
            $public_groups = $groupModel->find('list', array('conditions' =>
                $conditions,
                'fields' => array('Group.id')
            ));

            Cache::write($key,$public_groups,"group");
        }

        $groupUserModel = MooCore::getInstance()->getModel("Group.GroupUser");
    	$groups = $groupUserModel->getMyGroupsList( $event->data['param'] );

        $groups = array_unique(array_merge($public_groups,$groups));
